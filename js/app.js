...
// design (this file is a straight geometry reuse), but:
//   1. Union (tab+body) and the front panel both use the SAME sky-blue
//      gradient (#96CEF7 -> #148DE4, a plain 2-stop top-to-bottom —
//      no 34%-hold like the old maroon version had).
//   2. The original's flat-color "band" layer is gone — node 29:143
//      has no equivalent shape at all; the paper rects (below) sit
//      directly over the Union outline's own blue gradient instead of
//      a separate backdrop.
//   3. Three light-grey, half-opaque paper rects (#E6E4E4 @ 50%,
//      exact Figma path data, positioned at their own absolute
//      offsets) sit between the outline and the front panel — visible
//      at REST now (peeking above the panel), not just during the
//      tap-open animation like the old design's papers were.
// Label/count/icon ink flips to white — the front panel is now a
// medium blue, not a light cream, so the old maroon-on-cream text
// color would be unreadable.
const TILE_GRAD_START = '#96CEF7';
const TILE_GRAD_END = '#148DE4';
const TILE_INK = '#FFFFFF'; // label/count/icon — white, readable on the blue front panel
function tileStyle(_cat){
  return `--tile-grad-start:${TILE_GRAD_START}; --tile-grad-end:${TILE_GRAD_END}; --tile-ink:${TILE_INK};`;
}

// Same layer structure/path data as the original folder (viewBox
// 199.667x181.943, preserveAspectRatio="none") — see the palette
// comment above for what changed and why. The three paper rects use
// their own exact Figma path data (node 29:143's Rectangle 15/10/14),
// each positioned via its own absolute translate rather than nested
// inside the front panel's — that's how Figma laid them out, and it's
// also what lets two of them (see .folder-paper-1/-2 in style.css)
// keep participating in the tap-open animation unchanged: a CSS
// translateY is relative motion, so reusing those classes here needed
// no changes to the keyframes themselves.
...
function folderSvg(){
  ...
  return `<svg class="folder-svg" viewBox="0 0 199.667 181.943" preserveAspectRatio="none" aria-hidden="true">
    ...
    <!-- Figma's actual node (29:143) has no separate "band" shape here
         at all — the three paper rects below sit directly over the
         Union outline's own blue gradient (it already spans the full
         height). An earlier pass invented a cream backdrop for them,
         reusing this app's original maroon-theme structure (which DID
         have a real band layer, at this same translate) — that read
         as a beige/tan stripe instead of Figma's cool pale-blue-grey
         paper fan, because 50%-opacity light-grey paper looks very
         different sitting on cream versus sitting on blue. Removed
         entirely rather than recolored, to match the source exactly.

         The papers themselves are solid, not translucent, in Figma —
         despite the raw SVG export's fill-opacity:0.5 on #E6E4E4 (what
         the previous pass used verbatim). Sampling Figma's own
         rendered pixels directly (a Dev Mode screenshot, not the flat
         vector export) showed each step is a distinct near-opaque
         tone, and the per-channel math doesn't reduce to any single
         alpha blended over the blue behind it — Figma is almost
         certainly using a background-blur/frosted-glass layer effect
         there, which a flat SVG fill can only ever approximate, never
         reproduce exactly (and backdrop-filter itself isn't reliable
         on raw SVG shapes across browsers, especially iOS Safari,
         where this app actually runs as an installed PWA). So instead
         of chasing the blur mechanism, these are the three sampled
         colors themselves, used as solid(ish) fills — genuinely
         opaque like the reference, not "translucent gray over blue"
         like the previous pass. -->
    <g transform="translate(29,27)"><path class="folder-paper" d="M0.00142857 12.1831C-0.101772 5.42143 5.39988 -0.0900308 12.1618 0.00111444L127.79 1.55969C134.426 1.64915 139.729 7.10859 139.626 13.7447L138.211 105.021C138.109 111.575 132.767 116.835 126.212 116.835H13.4169C6.86091 116.835 1.51837 111.573 1.41832 105.018L0.00142857 12.1831Z" fill="#C5D5E2"/></g>
    <!-- The outer translate() here is a plain SVG attribute, kept OFF
         the animated element itself — a CSS transform on a node that
         already has its own transform attribute doesn't compose with
         it reliably, so positioning (attribute, static) and animation
         (CSS class, inner element with no attribute of its own) are
         deliberately split across a wrapper/child pair, same pattern
         as .folder-front below. -->
    <g transform="translate(21,33)"><g class="folder-paper-1"><path class="folder-paper" d="M0.00179627 12.2054C-0.113964 5.44201 5.38039 -0.0804136 12.1443 0.000886302L144.727 1.59449C151.379 1.67445 156.696 7.15073 156.581 13.8023L154.993 105.061C154.879 111.606 149.541 116.852 142.995 116.852H13.5893C7.04191 116.852 1.70309 111.604 1.59104 105.058L0.00179627 12.2054Z" fill="#DADDE0"/></g></g>
    <g transform="translate(15,41)"><g class="folder-paper-2"><path class="folder-paper" d="M0.00208112 12.2211C-0.122564 5.45572 5.36795 -0.0747868 12.134 0.000764995L156.684 1.61485C163.347 1.68924 168.673 7.17708 168.548 13.8388L166.839 105.087C166.717 111.626 161.381 116.863 154.841 116.863H13.7109C7.16968 116.863 1.83348 111.624 1.71299 105.084L0.00208112 12.2211Z" fill="#E3E1E1"/></g></g>
    <g transform="translate(2,58)">
      <!-- The front panel, hinged at its own bottom edge — on tap-open
           it lifts back (see folder-flap-open in style.css) like a
           real folder cover opening, instead of squashing flat. -->
...
// A real frosted-glass version of the same three paper steps, as HTML
// divs layered over the SVG rather than more SVG shapes — this is
// what actually gets Figma's effect (see the big comment on the SVG
// papers above for why a flat SVG fill can only approximate it).
// backdrop-filter isn't reliably usable on raw SVG shapes across
// browsers, but is well-supported on ordinary HTML elements — including
// iOS Safari specifically, which matters here since this app runs
// there as an installed PWA. Only used where a tile is large enough
// for a 6-8px blur to read as glass, not noise — the small 34px
// .sublist-icon folders keep the plain SVG paper fill above instead
// (see folderSvg()).
//
// Each step is TWO nested divs, not one:
//   .paper-glass-crop  — sized/positioned to the exact percentage of
//                        the tile visible above the front panel's own
//                        top edge (y=58 in the 199.667x181.943
//                        viewBox) — there's no point rendering
//                        further down, since the opaque front panel
//                        covers that regardless — and clips to it
//                        (overflow:hidden).
//   .paper-glass-fill   — inside that, sized to the paper's FULL
//                        natural height (not just the visible slice),
//                        so the clip-path below (the shared, exact
//                        Figma corner shape — see index.html's hidden
//                        <svg><defs>) isn't vertically squashed.
//                        objectBoundingBox clip-path units scale to
//                        fit WHATEVER box references them; giving it
//                        the true full-height box first, then cropping
//                        the visible window with the OUTER div, keeps
//                        the corner curve proportioned correctly
//                        instead of distorted into the short sliver
//                        that's actually visible.
// All percentages are of the tile itself (not %-of-parent) so this
// still lines up correctly however these two nested boxes stack —
// computed from the same Figma coordinates as the SVG papers.
function folderPapersGlass(){
  return `<div class="folder-papers-glass" aria-hidden="true">
    <div class="paper-glass-crop paper-glass-crop-a"><div class="paper-glass-fill paper-glass-fill-a"></div></div>
    <div class="paper-glass-crop paper-glass-crop-b"><div class="paper-glass-fill paper-glass-fill-b"></div></div>
    <div class="paper-glass-crop paper-glass-crop-c"><div class="paper-glass-fill paper-glass-fill-c"></div></div>
  </div>`;
}
...
function renderCard(p){
  // theme2: collection tags now get a color too, same hash-based hue
  // system as the main category pill (catStyle()) — was plain grey
  // before, on every theme.
  const colls = p.collections.slice(0,3).map(c => `<span class="collchip" style="${catStyle(c)}">${escapeHtml(c)}</span>`).join('');
